...
public class Normal_slots extends AppCompatActivity implements JsonResponse{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        JR.json_response = (JsonResponse) Normal_slots.this;
        String q = "/view_slot?location="+Parking_loc.loc_id;
        q = q.replace(" ", "%20");
        JR.execute(q);
...
//                        val[i]="Fuel Type : "+fuel_type[i]+"\nVehicle : "+vehicle[i]+"\nReg.No : "+regnum[i]+"\nDriver Name : "+dname[i]+"\nPhone : "+phone[i]+"\nEmail : "+email[i];
                    ...
                    Custom_slot ar = new Custom_slot(Normal_slots.this, name, amount);
                    l1.setAdapter(ar);
...
//            }
                    l1.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                        ...
                        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                            ...
                            DatePickerDialog datePickerDialog = new DatePickerDialog(Normal_slots.this, new DatePickerDialog.OnDateSetListener() {
                                @Override
                                public void onDateSet(DatePicker view, int year, int month, int dayOfMonth) {
                                    ...
                                    AlertDialog.Builder builder = new AlertDialog.Builder(Normal_slots.this);
                                    builder.setTitle("ENTER VEHICLE NUMBER");

                                    // Set up the input
                                    final EditText input = new EditText(Normal_slots.this);
                                    input.setInputType(InputType.TYPE_CLASS_TEXT);
                                    builder.setView(input);
